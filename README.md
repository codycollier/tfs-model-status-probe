...
`tfs_model_status_probe` checks the model status in a TensorFlow Serving [1] instance

The `tfs_model_status_probe` is modeled after `grpc_health_probe` [2] and is intended for use as a kubernetes probe for a TensorFlow Serving service. It calls the ModelService.GetModelStatus() rpc [3] for a given model and returns a response code indicating success.  If the model is `AVAILABLE`, then the probe will have an exit code of 0 (conforming to k8s probe conventions).  If the model is still `LOADING`, in some other state, or there are grpc communication errors then the exit code will be non-zero.

If no version is provided, the probe assumes a single version is loaded and just checks the first version in the response.
...
[2] https://github.com/grpc-ecosystem/grpc-health-probe
...
[3] https://github.com/tensorflow/serving/blob/master/tensorflow_serving/apis/model_service.proto
